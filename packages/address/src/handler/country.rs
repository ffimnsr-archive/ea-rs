...
async fn retrieve(req: Request<Body>) -> ServiceResult<Response<Body>> {
    ...
    let collection = db.collection::<Country>(DEFAULT_COLLECTION);
    let cursor = collection
        .find(None, None)
        .await
        .map_err(ServiceError::Mongo)?;
    let countries = cursor
        .try_collect::<Vec<Country>>()
        .await
        .map_err(ServiceError::Mongo)?;
    let output = serde_json::to_string(&countries).map_err(ServiceError::SerdeJson)?;
...
async fn find_one(req: Request<Body>) -> ServiceResult<Response<Body>> {
    let db = req.data::<Arc<Database>>().ok_or(ERR_DB_INSTANTIATE)?;
    let collection = db.collection::<Country>(DEFAULT_COLLECTION);
    let country = collection
        .find_one(None, None)
        .await
        .map_err(ServiceError::Mongo)?
        .ok_or(ServiceError::DefinedError("Unable to find mongo record"))?;

    let output = serde_json::to_string(&country).map_err(ServiceError::SerdeJson)?;
