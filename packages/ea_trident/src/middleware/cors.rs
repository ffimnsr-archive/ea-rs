use hyper::header::{self as HeaderKey, HeaderValue};
    ...
    E: Error + Send + Sync + Unpin + 'static,
{
    ...
    headers.insert(
        HeaderKey::ACCESS_CONTROL_ALLOW_ORIGIN,
    ...
    headers.insert(
        HeaderKey::ACCESS_CONTROL_ALLOW_METHODS,
        HeaderValue::from_static("OPTIONS, HEAD, GET, POST"),
    );

    // Allow only this requests headers to be used.
    headers.insert(
        HeaderKey::ACCESS_CONTROL_ALLOW_HEADERS,
        HeaderValue::from_static(
            "authorization, content-type, accept, origin, user-agent",
        ),
    );

    // Allow this headers to be accessed by client on javascript to
    // be parsed from response.
    headers.insert(
        HeaderKey::ACCESS_CONTROL_EXPOSE_HEADERS,
        HeaderValue::from_static("content-encoding, x-session-id"),
    );

    // Set how long the results of preflight request can be cached.
    headers.insert(
        HeaderKey::ACCESS_CONTROL_MAX_AGE,
        HeaderValue::from_static("7200"),
    );

    // Used as a security measure to prevent adding wildcards and or
    // additions to the allowed origins specially if the origin contains
    // multiple domains.
    headers.insert(HeaderKey::VARY, HeaderValue::from_static("Origin"));
