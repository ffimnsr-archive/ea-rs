...
pub async fn retrieve(req: Request<Body>) -> ServiceResult<Response<Body>> {
    ...
    let accounts = cursor
        ...
        .await
        .map_err(ServiceError::Mongo)?;
    let output = serde_json::to_string(&accounts).map_err(ServiceError::SerdeJson)?;
...
pub async fn find_one(req: Request<Body>) -> ServiceResult<Response<Body>> {
    ...
    let output = serde_json::to_string(&account).map_err(ServiceError::SerdeJson)?;
...
fn router(db: Arc<Database>) -> ServiceResult<Router<Body, ServiceError>> {
    ...
    Router::<Body, ServiceError>::builder()
        ...
        .get("/api/accounts", retrieve)
        .post("/api/accounts", create)
        .get("/api/accounts/:id", find_by_id)
...
async fn main() -> ServiceResult<()> {
    ...
    if env::var("RUST_LOG").is_err() {
        env::set_var("RUST_LOG", "ea_account=trace,ea_trident=trace");
    }
